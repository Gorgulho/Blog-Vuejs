const microposts = {
    ...
    actions: {
        async getMicroposts({commit}) {
			try {
				const response = await fetch(`http://daw.deei.fct.ualg.pt/~a71342/LAB8_10/api/microposts.php`)
        ...
        async addMicropost({commit, rootGetters}, post) {
			try {
				const response = await fetch(`http://daw.deei.fct.ualg.pt/~a71342/LAB8_10/api/microposts.php?session_id=${rootGetters['user/getUser'].session_id}`, {
        ...
        async updateMicropost({commit, rootGetters}, post) {
			try {
				const response = await fetch(`http://daw.deei.fct.ualg.pt/~a71342/LAB8_10/api/microposts.php?micropost_id=${post.id}&session_id=${rootGetters['user/getUser'].session_id}`, {
		...
		async deleteMicropost({commit, rootGetters},id) {
			try {
				await fetch(`http://daw.deei.fct.ualg.pt/~a71342/LAB8_10/api/microposts.php?micropost_id=${id}&session_id=${rootGetters['user/getUser'].session_id}`, {
