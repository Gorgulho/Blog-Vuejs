const user = {
    ...
    actions: {
        async userExists({commit}, user) {
			try {
				const response = await fetch(`http://daw.deei.fct.ualg.pt/~a71342/LAB8_10/api/users.php?email=${user.email}`, {
        ...
        async addUser({state}) {
			try {
                const response = await fetch('http://daw.deei.fct.ualg.pt/~a71342/LAB8_10/api/users.php', {
        ...
        async loginUser({commit}, user) {
			try {
				const response = await fetch(`http://daw.deei.fct.ualg.pt/~a71342/LAB8_10/api/users.php?email=${user.email}&password=${user.password}`)
        ...
        async logoutUser({commit},session_id) {
			try {
				const response = await fetch(`http://daw.deei.fct.ualg.pt/~a71342/LAB8_10/api/users.php?session_id=${session_id}`)
